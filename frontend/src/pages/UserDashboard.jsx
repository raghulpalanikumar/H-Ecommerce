                    ...
                    <table className="dash-table">
                      ...
                      <tbody>
                        {orders.slice(0, 5).map(o => (
                          <tr key={o.id}>
                            <td style={{ fontWeight: 600 }}>#{String(o.id).slice(-6).toUpperCase()}</td>
            ...
            <div className="dash-table-container">
              <table className="dash-table">
                ...
                <tbody>
                  {orders.map(o => (
                    <tr key={o.id}>
                      <td style={{ fontWeight: 700, color: '#0066cc' }}>#{String(o.id).slice(-8).toUpperCase()}</td>
                      <td>{formatDate(o.date)}</td>
                      <td>
                        <div style={{ fontWeight: 600, color: '#1e293b' }}>
                          {o.items.map(item => item.name).slice(0, 2).join(', ')}
                          {o.items.length > 2 ? ' ...' : ''}
                        </div>
                        <div style={{ fontSize: '0.75rem', color: '#64748b' }}>{o.items.length} item(s) total</div>
                      </td>
                      <td style={{ fontWeight: 700 }}>{formatPrice(o.total)}</td>
                      <td><span className={`dash-badge badge-${o.status.toLowerCase()}`}>{o.status}</span></td>
